...
describe('Book Store app', () => {
  ...
  it('user should be able to login', () => {
    cy.findPlaceholder('UserName').type(testData.user.username);
    cy.findPlaceholder('Password').type(testData.user.password);
    cy.get('#login').click();
    cy.get('#userName-value').should('contain', testData.user.username);
    cy.url().should('include', '/profile');
  });

  it('user should be able to add book to shopping list', () => {
